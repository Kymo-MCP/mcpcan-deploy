...
2. **Initialize Environment Configuration**:
   ...
   _(Optional) Use a text editor (such as `vim` or `nano`) to modify configurations in the `.env` file, for example, modifying the default port `MCP_ENTRY_SERVICE_PORT`._
...
3. **Generate Service Configuration**:
   ...
   ./replace.sh
   ```
   _Note: If you modify `.env` later, you must re-run this script to apply the changes._
...
3. **Core Services Start**: After MySQL and Redis are healthy, core services like `mcp-authz` and `mcp-market` start.
4. **Access Layer Start**: Finally, `mcp-web` and the `traefik` gateway start to provide external services.
...
- **HTTPS Access**: [https://localhost](https://localhost) (or your configured HTTPS port)
  - _Note: A self-signed certificate is used by default. The browser will prompt that it is insecure; please click "Proceed" to continue._
...
Ensure all service statuses are `Up` (or `Up (healthy)`).
...
# View specific service logs (e.g., mcp-market)
...
| **mcp-authz**  | Authentication & Authorization Service              | Depends on database health        |
| **mcp-market** | Plugin Market Core Service                          | Depends on database health        |
